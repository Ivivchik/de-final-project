INSERT INTO IVIVCHIKYANDEXRU__DWH.s_transaction_country(hk_transaction_country_pk,
                                                        h_transaction_pk,
                                                        country,
                                                        load_dt,
                                                        load_src)
    ...
    SELECT tt.*
	  ...
	  SELECT DISTINCT HASH(operation_id || country),
                      HASH(operation_id),
                      country,
                      NOW(),
                      's3'
                 FROM grouped_transaction t 
                WHERE HASH(operation_id || country) NOT IN (
                      SELECT hk_transaction_country_pk
                        FROM IVIVCHIKYANDEXRU__DWH.s_transaction_country)
